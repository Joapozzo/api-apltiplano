...
export class UploadController {
  static async subirImagen(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const focalX =
        typeof req.body.focal_x === "string" || typeof req.body.focal_x === "number"
          ? Number(req.body.focal_x)
          : undefined;
      const focalY =
        typeof req.body.focal_y === "string" || typeof req.body.focal_y === "number"
  ...
  static async actualizarFocal(req: Request, res: Response, next: NextFunction) {
    try {
      const idServicio = Number.parseInt(parseParamId(req.params.id_servicio), 10);
