...
import {
  encryptInscripcionPii,
  encryptDatosMedicosFields,
...
function parseIdNumber(value: unknown, fieldName: string): number {
  const num = typeof value === "string" ? parseInt(value, 10) : Number(value);
  if (isNaN(num)) throw new AppError(`${fieldName} debe ser un número válido`, 400);
  return num;
}

export class InscripcionesController {
  static generateLink = asyncHandler(async (req: Request, res: Response) => {
    const { id_expedicion, id_cliente, expires_in_days } = req.body;

    if (!id_expedicion) throw new AppError("id_expedicion es requerido", 400);
  ...
  static submit = asyncHandler(async (req: Request, res: Response) => {
    ...
    const usuario = {
      ...
      telefono: inscripcionPii.telefono as string,
      provincia: inscripcionPii.provincia as string,
      localidad: inscripcionPii.localidad as string,
      nacionalidad: inscripcionPii.nacionalidad as string,
      emergencia_nombre: inscripcionPii.emergencia_nombre as string,
      emergencia_telefono: inscripcionPii.emergencia_telefono as string,
      emergencia_vinculo: inscripcionPii.emergencia_vinculo as string,
    };

    const dm = datosMedicosEncrypted as Record<string, unknown>;
    ...
    const result = await InscripcionesService.submitInscripcion({
      ...
      datos_medicos: {
        ...
        restricciones_alimentarias: (dm.restricciones_alimentarias as string) ?? data.datos_medicos.restricciones_alimentarias,
        operaciones: (dm.operaciones as string) ?? data.datos_medicos.operaciones,
        lesiones: (dm.lesiones as string) ?? data.datos_medicos.lesiones,
  ...
  static list = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.query.expedicion) filters.expedicion = parseInt(req.query.expedicion as string, 10);
    if (req.query.cliente) filters.cliente = parseInt(req.query.cliente as string, 10);
    if (req.query.activas === "true") filters.activas = true;
    if (req.query.detalle === "true") filters.detalle = true;
  ...
  static reembolsar = asyncHandler(async (req: Request, res: Response) => {
    const id = parseParamId(req.params.id);
    if (!id) throw new AppError("ID es requerido", 400);
  ...
  static listTokens = asyncHandler(async (req: Request, res: Response) => {
    const filters: {
    ...
    if (req.query.id_expedicion) filters.id_expedicion = parseInt(req.query.id_expedicion as string);
    if (req.query.usado === "true" || req.query.usado === "false") filters.usado = req.query.usado === "true";
  ...
  static disableToken = asyncHandler(async (req: Request, res: Response) => {
    const id = parseParamId(req.params.id);
    if (!id) throw new AppError("ID es requerido", 400);
