...
export async function getDashboardResumen(filtros: DashboardFiltros): Promise<DashboardResumenResponse> {
  const fechaDesde = filtros.fecha_desde ?? new Date(new Date().getFullYear(), new Date().getMonth(), 1);
  const fechaHasta =
    filtros.fecha_hasta ?? new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0, 23, 59, 59, 999);
  const hoy = new Date();
