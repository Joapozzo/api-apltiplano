import { createNota, deleteNota, getNotaById, getNotas, getNotasByMes, toggleCompletada, updateNota, } from "../services/notas-calendario.service.js";
...
export class NotasCalendarioController {
    static getNotas = asyncHandler(async (req, res) => {
    ...
    static getNotasByMes = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
    ...
    static getNotaById = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
    ...
    static createNota = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
    ...
    static updateNota = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
        const id = Number(req.params.id);
        if (!Number.isInteger(id))
            throw new AppError("ID de nota inválido", 400);
    ...
    static toggleCompletada = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
        const id = Number(req.params.id);
        if (!Number.isInteger(id))
            throw new AppError("ID de nota inválido", 400);
    ...
    static deleteNota = asyncHandler(async (req, res) => {
        const idUsuario = getAuthenticatedUserId(req);
        const id = Number(req.params.id);
        if (!Number.isInteger(id))
            throw new AppError("ID de nota inválido", 400);
...
//# sourceMappingURL=notas-calendario.controller.js.map