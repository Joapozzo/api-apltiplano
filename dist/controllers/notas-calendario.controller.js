...
function handleNotasError(error, res, fallbackMessage) {
...
export class NotasCalendarioController {
    static async getNotas(req, res) {
        try {
            ...
            const completada = typeof req.query.completada === "string"
    ...
    static async getNotasByMes(req, res) {
        try {
            const idUsuario = getAuthenticatedUserId(req);
    ...
    static async toggleCompletada(req, res) {
        try {
            const idUsuario = getAuthenticatedUserId(req);
            const id = Number(req.params.id);
            if (!Number.isInteger(id)) {
