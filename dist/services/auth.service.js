...
export class AuthService {
    ...
    static async registerWithFirebaseToken(idToken, input) {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingByUid && existingByUid.email !== email) {
                ...
                if (emailConflict && emailConflict.id_usuario !== existingByUid.id_usuario) {
                    throw new AuthServiceError("El email de Firebase ya estA asociado a otro usuario local", 409, "EMAIL_ALREADY_IN_USE");
                }
            }
            const userData = {
            ...
            if (existingByEmail) {
                if (existingByEmail.firebase_uid && existingByEmail.firebase_uid !== firebaseUid) {
                    throw new AuthServiceError("Ya existe un usuario local con ese email vinculado a otro UID de Firebase", 409, "FIREBASE_UID_MISMATCH");
                }
                const updatedUser = await tx.usuarios.update({
    ...
    static async authenticateRequest(idToken) {
        const decoded = await this.verifyIdToken(idToken);
        const user = await prisma.usuarios.findUnique({
            where: { firebase_uid: decoded.uid },
            select: userMeSelect,
